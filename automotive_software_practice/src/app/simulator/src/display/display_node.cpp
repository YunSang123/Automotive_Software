 ...
 *              : add missions visualization function
 */
...
Display::Display(const std::string& node_name, const double& loop_rate,
    ...
    s_obstacles_ = this->create_subscription<ad_msgs::msg::Obstacles>(
        "obstacles", qos_profile, std::bind(&Display::CallbackObstacles, this, std::placeholders::_1)); // Changed message type
    ...
    p_ego_vehicle_velocity_ = this->create_publisher<std_msgs::msg::Float32> (
     ...
     p_obstacles_marker_ = this->create_publisher<visualization_msgs::msg::MarkerArray> (
        "obstacles_marker", qos_profile);
...
void Display::Run(const rclcpp::Time& current_time) {
    ...
    //     time_vehicle_marker_ = current_time.seconds();
        ...
        DisplayObstacles(obstacles, vehicle_state, current_time);
...
void Display::DisplayObstacles(const ad_msgs::msg::Obstacles& obstacles,
                              const ad_msgs::msg::VehicleOutput& vehicle_state,
                              const rclcpp::Time& current_time) {
    ...
    for (auto obstacle : obstacles.obstacles) {
        ...
        obstacle_marker.ns = obstacle.id;
        ...
        tf2::Quaternion q;
        q.setRPY(0, 0, obstacle.yaw + M_PI);
        q.normalize();
        obstacle_marker.pose.orientation.x = q.x();
        obstacle_marker.pose.orientation.y = q.y();
        obstacle_marker.pose.orientation.z = q.z();
        obstacle_marker.pose.orientation.w = q.w();
        ...
        double v_x = vehicle_state.x;
        double v_y = vehicle_state.y;
        double o_x = obstacle.x;
        double o_y = obstacle.y;

        double det_len = sqrt((v_x - o_x)*(v_x - o_x) + (v_y - o_y)*(v_y - o_y));
        ...
        if(det_len > 26){
            ...
            obstacle_marker.color.r = 0.5 - 0.5 * (det_len - 13) / 13;
            obstacle_marker.color.g = 0.5 + 0.5 * (det_len - 13) / 13;
            obstacle_marker.color.b = 0.0;
        } else {
    ...
    p_obstacles_marker_->publish(obstacles_marker_array);
}
